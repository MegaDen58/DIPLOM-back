...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    @Query(value = "SELECT p.id, p.name, p.images, p.description, p.price, p.winter, p.summer, p.color, p.material, p.size FROM products p JOIN order_items oi ON p.id = oi.items WHERE oi.order_id = :orderId", nativeQuery = true)
